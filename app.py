...
import folium
from streamlit_folium import folium_static  # st_foliumからfolium_staticに変更
import requests
import json
...
def get_location_from_city(city_name):
    ...
    try:
        ...
        st.error("タイムアウトエラー: もう一度お試しください。")
        return None
    except GeocoderServiceError as e:
        st.error(f"ジオコーディングサービスエラー: {str(e)}")
        return None
    except Exception as e:
        st.error(f"エラーが発生しました: {str(e)}")
        return None
